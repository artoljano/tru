...
const socialLinks = [
  ...
  {
    Icon: FaPatreon,
    url: "https://www.patreon.com/YourPage",
...
function Layout() {
  ...
  // Albanian labels, paths stay the same
  const navItems = [
    { label: "Episodet", path: "/episodes" },
    { label: "Rreth Nesh", path: "/about" },
    { label: "Tru Media", path: "/newsletter" },
    { label: "Sygjero", path: "/suggest" },
    { label: "Vlerëso", path: "/review" },
  ];
    ...
    <div className="min-h-screen bg-black">
      ...
      >
        ...
        <div className="flex items-center space-x-5">
          {socialLinks.map(({ Icon, url, hoverClass }, idx) => (
            <motion.a
              key={idx}
              href={url}
              target="_blank"
              ...
              whileTap={{ scale: 0.9 }}
              className={`text-white transition-colors ${hoverClass}`}
            >
              <Icon size={22} />
            </motion.a>
          ))}
      ...
      {/* Mobile Navbar */}
      <nav className="md:hidden fixed w-full z-50 bg-blue-900/20">
          ...
          {isMenuOpen && (
            ...
            >
              <div className="flex flex-col items-center space-y-8">
                {navItemsMobile.map(({ label, path }, idx) => (
                  <motion.button
                    key={idx}
                    whileTap={{ scale: 0.95 }}
